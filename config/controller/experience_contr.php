    ...
    class experienceControl {
        ...
        public function handle(): void {
            ...
            if (isset($this->postData['delete'])) {
                // DB querry (only after validation)
                $trash = $model->deleteExperience($this->postData['experience_id']);
                if (!$trash) {
                    // Hold error message + set previous UI state
                    $_SESSION['error'] = 'Failed to delete work experience.';
                    ViewBook::revert($this->postData['action'] ?? '');
                    return;
                }

                $_SESSION['success'] = 'Work experience removed.';
            ...
            if (empty($this->postData['experience_id'])) {
