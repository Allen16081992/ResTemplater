<?php
    ...
    class SessionBook {  
        ...
        public static function logoutUser(): void {
            ...
            // FORCE the browser back to the absolute domain root, dropping all folder layers
            header('Location: /');
            exit;
        }
        ...
        public static function enforceToken(): void {
            ...
            if (!is_string($token) || !hash_equals($_SESSION['csrf_token'] ?? '', $token)) {
                ...
                // Check if this was a "fake" mismatch caused by an expired session
                if (empty($_SESSION['csrf_token'])) {
                    // Heal the system: boot a fresh token immediately
                    self::invokeToken(); 
                    $_SESSION['error'] = 'Your session timed out due to inactivity.';
                } else {
                    // This was a real malicious mismatch or heavily corrupted state
                    $_SESSION['error'] = 'Security token mismatch. Please reload the page.';
                }
